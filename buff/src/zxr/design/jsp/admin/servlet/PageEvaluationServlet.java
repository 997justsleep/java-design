...
@WebServlet("/admin/pagingEvaluation")
public class PageEvaluationServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        List<Evaluation> evaluationList = EvaluationDaoImpl.getInstance().selectAll(currentPage, PAGE_SIZE);

        int totalCount = EvaluationDaoImpl.getInstance().getTotalShareCount();
        int totalPages = (totalCount + PAGE_SIZE - 1) / PAGE_SIZE; // 计算总页数
        System.out.println("totalCount: "+totalCount+",totalPages: "+totalPages);

        req.getSession().setAttribute("evaluationList", evaluationList);
        req.getSession().setAttribute("currentPage", currentPage);
        req.getSession().setAttribute("totalPages", totalPages);

        req.getRequestDispatcher("evaluation.jsp").forward(req, resp);
    }
}
